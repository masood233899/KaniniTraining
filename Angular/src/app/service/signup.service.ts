...
export class SignupService {
  ...
  return this.http.post<LoginResponseModel>(`https://localhost:7281/api/Authorization/Login`,model);
  }

  signup(model:SignupRequestModel){
     return this.http.post<Status>(`https://localhost:7281/api/Authorization/Registration`,model);
  }

  chagePassword(model:ChangePasswrd){
    return this.http.post<Status>(`https://localhost:7281/api/Authorization/ChangePassword`,model);
    }
